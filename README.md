...
- The admin panel reaches Docker through `tecnativa/docker-socket-proxy` with
  a narrow allowlist (list / inspect / restart containers, stream logs). It
  cannot create containers, exec into them, or mount host paths — so an
  admin-panel compromise stays contained to the compose project rather than
  escalating to host root.
