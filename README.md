...
- The admin panel reaches Docker through `tecnativa/docker-socket-proxy` with
  a narrow allowlist (`CONTAINERS=1`, `POST=1`, `PING=1`, `VERSION=1`). This
  blocks `EXEC`, `IMAGES`, `VOLUMES`, `NETWORKS`, `SWARM`, container `DELETE`,
  and the rest of the Docker API surface. It does still permit POST verbs on
  `/containers/*` (which is what we use for `containers.restart`), and that
  category includes `containers.create` + `containers.start` — so a
  determined attacker with admin RCE could still create a privileged
  container with a bind-mounted host filesystem and escape to host root that
  way. The proxy substantially reduces blast radius vs the raw socket, but
  treat admin compromise as host compromise for threat-modeling.
